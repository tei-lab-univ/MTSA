...
本リポジトリは [lafhisによるMTSA](https://git.exactas.uba.ar/lafhis/mtsa) からのカスタムフォークです。
研究室独自の拡張内容については、[CHANGELOG.md](./CHANGELOG.md) に集約されています。

開発に参加する場合や、独自機能の詳細を確認したい場合は、まずこちらを参照してください。

## 開発と貢献について

ブランチ運用（`main` や `sync` の保護ルール）や、本家との同期手順については [CONTRIBUTING.md](./CONTRIBUTING.md) にまとめています。
