...
public class MTSUtilsTest extends MTSTestBase {
	...
	public void testIsMTSRepresentation() throws Exception {
		...
		assertFalse(MTSUtils.isMTSRepresentation(MTSToAutomataConverter.getInstance().convert(mts, "dipi", true, true)));
		
		mts.addTransition(ESTADO_CERO, "tau", ESTADO_CERO, TransitionType.MAYBE);
		assertTrue(MTSUtils.isMTSRepresentation(MTSToAutomataConverter.getInstance().convert(mts, "dipi", true, true)));
	}
