...
public class ProgressCheck {
	...
	public boolean doProgressCheck() {
	    boolean result = false;
		progress = true;
		output.outln("Progress Check...");
		long start = System.currentTimeMillis();
		ProgressTest.initTests(mach.getTransitionsLabels());
		stack = new Stack();
	...
	public void doLTLCheck() {
		...
		long start = System.currentTimeMillis();
		accept = acceptLabel(mach.getTransitionsLabels());
		if (accept < 0) {
	...
	private boolean component(MyHashProg hstk, Stack stack, byte[] currentState) {
		...
		BitSet hasActions = new BitSet(mach.getTransitionsLabels().length);
		// output.out("{");
		byte[] m;
	...
	private boolean missing(BitSet actions) {
		int alphalen = mach.getTransitionsLabels().length;
	...
	private void printSet(BitSet actions, boolean missing) {
		Vector v = new Vector();
		String[] alphabet = mach.getTransitionsLabels();
	...
	private void printCounterExample(Stack trace, byte[] root) {
		...
		output.outln("Violation of LTL property: "
				+ (mach.getTransitionsLabels())[accept]);
		output.outln("Trace to terminal set of states:");
		// printTrace(errorTrace);
	...
	Vector getCycleTrace(Vector component, byte[] root) {
		...
		while (!transitions.empty()) {
			...
			// output.outln("finding next "+(mach.getTransitionsLabels())[act]);
		...
		if (cycle != null) {
			...
			trace.add(0, (mach.getTransitionsLabels())[act]);
		}
		return trace;
	}
	...
	private boolean stateLabel(int act) {
		String s = (mach.getTransitionsLabels())[act];
		return s.charAt(0) == '_';
	}
