...
public class CompactState implements Automata {
    ...
    public CompactState[] components;
    public HashMap<Integer, int[]> stateToComponentStates;
    public HashMap<SimpleEntry<Integer, String>, int[]>  statePlusActionToComponentStates;
    ...
    public CompactState(String name, HashMap<Integer, String> stateToRole){
        this.name = name;
        this.stateToRole = stateToRole;
    }
    ...
    public CompactState(int size, String name, StateMap statemap, MyList transitions, String[] alphabet, int endSequence) {
        ...
        while (!transitions.empty()) {
            ...
            // info for ROLES feature for ML, there are no explicit submachines because it's an automatically generated CompactState
            stateToRole.put(fromState, name + "_s" + fromState);
    ...
    public CompactState(int size, String name, StateMap statemap, MyList transitions, String[] alphabet,
                        int endSequence, StateCodec coder,
                        HashMap<SimpleEntry<Integer, String>, int[]> statePlusActionToComponentStates,
                        CompactState[] components, HashMap<Integer, int[]> stateToComponentStates) {
        this(size, name, statemap, transitions, alphabet, endSequence, coder);
        this.statePlusActionToComponentStates = statePlusActionToComponentStates;
        this.stateToComponentStates = stateToComponentStates;
        this.components = components;
        this.stateToRole = null; // this is a composition // TODO refactor to one logic to rule them all
    }

    public HashMap<Integer, String> getStateToRoleMap() {
        return stateToRole;
    }

    public Boolean isComposition() {
        return components != null && components.length > 0
                && stateToComponentStates != null && !stateToComponentStates.isEmpty();
    }

    public ArrayList<Integer> getPrimitiveStates(Integer fromState, String actionName, Integer toState) {
        // Returns the primitive states representation for the given composition state 'toState'
        ArrayList<Integer> primitiveStatesList = new ArrayList<>();

        if(this.isComposition()){
            if (fromState == null){
                // We are getting the information for the initial state (there's not transitions leading to it)
                for (int i = 0; i < components.length; i++) {
                    CompactState component = components[i];
                    int componentToState = stateToComponentStates.get(toState)[i];
                    ArrayList<Integer> primitiveStatesForComponent = component.getPrimitiveStates(null, null, componentToState);
                    primitiveStatesList.addAll(primitiveStatesForComponent);
                }
            } else {
                // fromState should always exist in stateToComponentStates since it's never the deadlock (-1) state
                int[] componentStatesFrom = stateToComponentStates.get(fromState);
                int[] componentStatesTo = getComponentStates(fromState, actionName, toState);

                for (int i = 0; i < components.length; i++) {
                    CompactState component = components[i];
                    int componentFromState = componentStatesFrom[i];
                    int componentToState = componentStatesTo[i];

                    ArrayList<Integer> primitiveStatesForComponent = component.getPrimitiveStates(componentFromState, actionName, componentToState);
                    primitiveStatesList.addAll(primitiveStatesForComponent);
                }
            }
        } else {
            primitiveStatesList.add(toState);
        }

        return primitiveStatesList;
    }

    public ArrayList<String> getComponentsRole(Integer fromState, String actionName, int toState) {
        // Returns the roles representation for the given composition state 'toState'
        // That is, the role for each component in the composition, or an array with the unique role if it's not a composition
        ArrayList<String> roles = new ArrayList<>();

        if(this.isComposition()){
            if (fromState == null){
                // We are getting the information for a "normal" state (we don't need a transition to identify it)
                for (int i = 0; i < components.length; i++) {
                    CompactState component = components[i];
                    int componentToState = stateToComponentStates.get(toState)[i];
                    String componentRole = component.getRole(null, null, componentToState);
                    roles.add(componentRole);
                }
            } else {
                // fromState should always exist in stateToComponentStates since it's never the deadlock (-1) state
                int[] componentStatesFrom = stateToComponentStates.get(fromState);
                ...
                for (int i = 0; i < components.length; i++) {
                    ...
                    String componentRole = component.getRole(componentFromState, actionName, componentToState);
                    roles.add(componentRole);
                }
            }
        } else{
            roles.add(stateToRole.get(toState));
        }
        return roles;
    }

    public String getRole(Integer fromState, String actionName, int toState) {
        // Returns the role for 'toState'
        // The difference with getComponentsRole is that this method treats the CompactState as a single component
        // it combines the roles of the components into one using the delimiter specified in MonolithicFeatureCalculator
        String role;
        if(this.isComposition()){
            int[] componentStatesFrom = null;
            int[] componentStatesTo = null;

            if (fromState == null){
                // We are getting the information for a "normal" state (we don't need a transition to identify it)
                componentStatesTo = stateToComponentStates.get(toState);
            } else {
                // fromState should always exist in stateToComponentStates since it's never the deadlock (-1) state
                componentStatesFrom = stateToComponentStates.get(fromState);
                componentStatesTo = getComponentStates(fromState, actionName, toState);
            }
            role = buildRole(components, componentStatesFrom, actionName, componentStatesTo);
        } else {
            role = stateToRole.get(toState);
        }
        return role;
    }

    public static String buildRole(CompactState[] components, int[] componentStatesFrom, String action, int[] componentStatesTo){
        StringBuilder role = new StringBuilder();
        String delimiter = MonolithicFeatureCalculator.delimiterForComponentsRole;
        ...
        for (int i = 0; i < components.length; i++) {
            ...
            String componentRole = component.getRole(componentFromState, action, componentToState);
            role.append(componentRole).append(delimiter);
        }
        return role.toString();

    }

    public int[] getComponentStates(Integer fromState, String actionName, int toState) {
        int[] componentStates;
        ...
        if(stateToComponentStates.containsKey(toState)){
            componentStates = stateToComponentStates.get(toState);
        } else {
            SimpleEntry<Integer, String> key = new SimpleEntry<>(fromState, actionName);
            componentStates = statePlusActionToComponentStates.get(key);
        }
...
//                    index++;
    ...
    public CompactState myclone() {
        CompactState m = new CompactState(name);
        m.endseq = endseq;
        m.prop = prop;
        ...
        // information for ROLES feature for ML
        if(stateToRole != null)
            m.stateToRole = new HashMap<>(stateToRole);

        // information for ROLES feature for ML, when the CompactState is a composition of components
        if(statePlusActionToComponentStates != null && !statePlusActionToComponentStates.isEmpty()){
            m.components = components;
            m.statePlusActionToComponentStates = new HashMap<>(statePlusActionToComponentStates);
            m.stateToComponentStates = new HashMap<>(stateToComponentStates);
        }
