    ...
    protected CompositeState compose(Vector<Value> actuals) {
        ...
        for (Object o : machines) { // machines contains a mixture of two
            ...
            if (o instanceof CompactState)
                ...
                CompositeState cs = (CompositeState) o;
                if (isHeuristic || isCompositional) { // avoid computing the parallel composition in heuristic analysis
        ...
        c.isCompositional = isCompositional;
        ...
        if (c.makeController || c.checkCompatible || c.isPlant
                ...
                || c.isHeuristic || c.isMonolithicDirector || c.isCompositional || c.isPartialOrderReduction) {
            this.buildAndSetGoal(c);
        }
        c.setComponentAlphabet((Set<String>) computeAlphabet(this.getComponentAlphabet()));
        return c;
    }
