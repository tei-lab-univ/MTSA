...
public class Analyser implements Animator, Automata {
    ...
    public Analyser(CompositeState cs, LTSOutput output, EventManager eman,
        ...
        explorerContext.actionNames = actionName;
    ...
    private CompactState private_compose(boolean dohiding) {
        ...
        HashMap<AbstractMap.SimpleEntry<Integer, String>, int[]> statePlusActionToComponentStates = null;
        HashMap<Integer, int[]> stateToComponentStates = null;
        CompactState[] components = null;
        if (explorerContext.statePlusActionToComponentStates == null || explorerContext.statePlusActionToComponentStates.isEmpty()) {
            //it's a composition of one machine (entire Plant), the info is lost we need to recover it from the Plant
            if(cs != null && cs.env!=null){ // TODO see WHEN they are null and fix
                stateToComponentStates = cs.env.stateToComponentStates;
                statePlusActionToComponentStates = cs.env.statePlusActionToComponentStates;
                components = cs.env.components;
            }
        } else {
            stateToComponentStates = explorerContext.stateToComponentStates;
            statePlusActionToComponentStates = explorerContext.statePlusActionToComponentStates;
            components = explorerContext.sm;
        }

        CompactState c = new CompactState(explorerContext.stateCount, cs.name,
                compositionEngine.getExploredStates(), compTrans, actionName, explorerContext.endSequence, coder,
                statePlusActionToComponentStates, components, stateToComponentStates);
        if (dohiding) {
            if (cs.hidden != null) {
                if (!cs.exposeNotHide)
                    c.conceal(cs.hidden);
