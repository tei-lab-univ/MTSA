...
class StateMachine {
    ...
    public HashMap<Integer, String> stateToRole = new HashMap<>();
    String name;
    String kludgeName;
    ...
    private void make(ProcessSpec spec) {
        ...
        //for each k, v in the explicit states, add the v to the stateToRole map with k as the value
        // iterate over the explicit states in alphabetical order
        HashMap<Integer, TreeSet<String>> stateToRoleSet = new HashMap<>();
        for (String k : new TreeSet<>(explicit_states.keySet())) {
            int v = explicit_states.get(k);
            TreeSet<String> rolesSet = stateToRoleSet.containsKey(v) ? stateToRoleSet.get(v) : new TreeSet<>();
            rolesSet.add(k); //add the new role
            stateToRoleSet.put(v, rolesSet);
        }
        // iterate over the stateToRoleSet and extract the submachine names
        for (Integer v : stateToRoleSet.keySet()) {
            TreeSet<String> rolesSet = stateToRoleSet.get(v);
            String submachineName = spec.name.toString() + "_" + String.join("-", rolesSet);
            stateToRole.put(v, submachineName);
        }
        ...
        while(!transitionsCopy.isEmpty()){
            ...
            for(Transition t : transitionsCopy){
                if(stateToRole.containsKey(t.from)) {
                    if(!stateToRole.containsKey(t.to)) {
                        String submachineName = stateToRole.get(t.from);
                        stateToRole.put(t.to, submachineName);
                    }
