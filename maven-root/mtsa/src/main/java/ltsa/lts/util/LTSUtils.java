...
public class LTSUtils {
	...
	public static Object stateToString(int[] state, ModelExplorerContext ctx, int sourceMachineIndex) {
		//ctx.sm[sourceMachineIndex].alphabet
		...
		for (int i = 0; i < state.length-1; i++) {
			...
			sb.append(ctx.sm[i].name + ": " + state[i]);
			//sb.append(state[i]);
		}
		sb.append("]");
		return sb.toString();
