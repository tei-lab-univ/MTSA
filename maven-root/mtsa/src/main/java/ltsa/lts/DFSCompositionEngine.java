...
public class DFSCompositionEngine implements CompositionEngine {
	...
	public void processNextState() {
		...
		// TODO check this is a real transition or just "state"?
		// add information for ROLES feature of ML to context
		ctx.addStateToComponentStates(ctx.stateCount - 1, state);
	...
	public void pruneUnfinishedStates() {
		...
		int tauIndex= 0;
		for (int i = 0; i < ctx.actionNames.length; i++) {
			if (ctx.actionNames[i].equals("tau")) {
				tauIndex= i;
				break;
			}
		}
