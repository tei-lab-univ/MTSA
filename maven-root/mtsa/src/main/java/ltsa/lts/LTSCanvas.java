...
public class LTSCanvas extends JPanel implements Scrollable {
    ...
    public void draw_composite(int id, CompactState m, int[] last, int[] current, String name) {
        ...
        try {
        	...
        	ls = last == null? 0: m.getCompositionStateFrom(last);
        	cs = current == null ? 0 : m.getCompositionStateFrom(current);
        }
        catch (Exception e) {
			System.err.println("Error in draw_composite: " + e.getMessage());
			ls = 0;
			cs = 0;
		}
        ...
        if (drawMachine!=null) {
           ...
           if (drawMachine.getClass() == DrawCompositeMachine.class) {
               ...
               if (((drawMachine.selected == 0) && (cs == 0)) ||
            		   Arrays.stream(current).allMatch(value -> value == 0)) {
