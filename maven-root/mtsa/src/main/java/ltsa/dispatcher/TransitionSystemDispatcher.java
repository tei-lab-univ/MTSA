...
import MTSTools.ac.ic.doc.mtstools.model.operations.DCS.TranslateToRS.TranslateToSpectra;
...
public class TransitionSystemDispatcher {
    ...
    public static CompactState getOptimisticModel(CompactState compactState, LTSOutput output) {
        ...
        return MTSToAutomataConverter.getInstance().convert(mts, compactState.name, true);
    }
    ...
    public static CompactState getPessimistModel(CompactState composition) {
        ...
        return MTSToAutomataConverter.getInstance().convert(mts, composition.name, true);
    }
    ...
    private static void merge(CompositeState composition, LTSOutput ltsOutput) {
        if (composition.getComposition() == null) {
            ...
            composition.setComposition(MTSToAutomataConverter.getInstance().convert(mts, composition.name, true));
    ...
    public static void parallelComposition(CompositeState compositeState, LTSOutput ltsOutput) {
        if (MTSUtils.isMTSRepresentation(compositeState)) {
            ...
            CompactState convert = MTSToAutomataConverter.getInstance().convert(mts, compositeState.name, true, true);
        ...
        } else if (!compositeState.isHeuristic && !compositeState.isCompositional && !compositeState.isPartialOrderReduction) {
...
//                //System.out.println("\n" + composed);
    ...
    public static void applyPlusCROperator(CompositeState compositeState, LTSOutput ltsOutput) {
        if (compositeState.getComposition() == null) {
            ...
            compositeState.setComposition(MTSToAutomataConverter.getInstance().convert(merge, compositeState.name, true));
            ltsOutput.outln(""); // leave an empty line
    ...
    public static CompactState determinise(CompactState lts, LTSOutput ltsOutput) {
        ...
        if (MTSUtils.isMTSRepresentation(compactState)) {
            ...
            return MTSToAutomataConverter.getInstance().convert(mts, compactState.name,
                    MTSUtils.isMTSRepresentation(compactState), true);
        } else {
    ...
    public static void monolithicDirectorSynthesis(CompositeState compositeState, final LTSOutput output,
        ...
        if (controller != null) {
            CompactState compState = MTSToAutomataConverter.getInstance().convert(
                    new MTSAdapter<Long, String>(controller), compositeState.getName(), true);
    ...
    public static void hcs(CompositeState compositeState, final LTSOutput output, Statistics... stats) {
        ...
        if(goal.isNonBlocking()){
            ...
            dcs = new DirectedControllerSynthesisGR1<>();
        }
		...
		if (controller != null) {
			CompactState compState = MTSToAutomataConverter.getInstance().convert(
				new MTSAdapter<Long, String>(controller), compositeState.getName(), false);
    ...
    public static void minimise(CompositeState compositeState, LTSOutput ltsOutput) {
        ...
        if (MTSUtils.isMTSRepresentation(compositeState)) {
            ...
            if (compactState != null) {
                ...
                compositeState.setComposition(
                        MTSToAutomataConverter.getInstance().convert(mts, compositeState.getComposition().name, true));
            }
...
//                // System.out.println("si llega aca es de milagro!");
    ...
    private static CompactState buildBuchiFromProperty(CompositeState compositeState, LTSOutput ltsOutput) {
        ...
        return MTSToAutomataConverter.getInstance().convert(property, compactState.name, true);
    }
    ...
    public static CompactState getAbstractModel(CompactState compactState, LTSOutput output) {
        ...
        return MTSToAutomataConverter.getInstance().convert(abstractModel, compactState.name, true);
    }
    ...
    public static CompactState makeMTSConstraintModel(CompactState compactState, LTSOutput output) {
        ...
        output.outln("Constrained model generated for " + compactState.name + " in: "
                + (System.currentTimeMillis() - initialTime) + "ms.");
        return MTSToAutomataConverter.getInstance().convert(constrained, compactState.name, true);
    }
    ...
    public static void makePlant(CompositeState compositeState, LTSOutput output) {
        ...
        if (composition != null) {
            if (compositeState.goal != null) {
                ...
                compositeState
                        .setComposition(MTSToAutomataConverter.getInstance().convert(plant, compositeState.getName(), true));
            } else {
        ...
        if (composition != null) {
            ...
            if (!pred.evaluate(env)) {
                if (goal != null) {
                    ...
                    compositeState
                            .setComposition(MTSToAutomataConverter.getInstance().convert(det, composition.getName(), true));
                } else {
...
//                //System.out.println("Applying Reachability Prunning!");
    ...
    public static CompactState synthesise(CompositeState compositeState, ControllerGoal<String> goal,
        ...
        if (!isMTS && (c.isNonDeterministic() || c.hasTau())) {
            ...
            System.out.println("It is nondeterministic or has tau");
            if (c.hasTau()) {
            ...
            if (goal.isNonBlocking()) {
                ...
                if (synthesiseController != null) {
                    ...
                    // compositeState.name));
                    return MTSToAutomataConverter.getInstance().convert(temp, compositeState.name, isMTS, true);
                } else {
                    return null;
                }
            }
            ...
            if (synthesised == null) {
                ...
                CompactState compactState = MTSToAutomataConverter.getInstance().convert(plainController,
                        compositeState.getName(), isMTS, true);
            ...
            if (goal.getBuchi().size() > 0) {
                ...
                synthesised = facade.synthesiseControllerWithPlantInformation(plant, goal, c);
                // TODO move the feature calculation here and remove method synthesiseControllerWithPlantInformation
                ...
                if (synthesised == null) {
                    ...
                    /* in this case i have to reintroduce selfloops transitions */
                    Set<String> cleanedLabels = goal.getSelfLoopsUncontrollable().stream()
                            .filter(label -> label.startsWith("c_"))
                            .map(label -> label.substring(2)) // Eliminar "c_"
                            .collect(Collectors.toSet());

                    if(!goal.getSelfLoopsUncontrollable().isEmpty()){
                        reintroduceSelfLoopTransitionsIntoController(plainController, plant, goal.getSelfLoopsUncontrollable(), cleanedLabels);
                    }
            ...
            System.out.println("MTS control");
    ...
    /*
        this method reintroduce self-loop transitions of plant contained in selfLoopsUncontrollable set
        into plainController controller
     */
    private static void reintroduceSelfLoopTransitionsIntoController(MTS<Long, String> plainController,
                                                                     MTS<Long, String> plant,
                                                                     Set<String> selfLoopsUncontrollable, Set<String> cleanedLabels) {

        Queue<Pair<Long, Long>> queue = new LinkedList<>();
        Set<Pair<Long, Long>> visited = new HashSet<>();

        queue.add(new Pair(0L, 0L));
        visited.add(new Pair(0L, 0L));

        while (!queue.isEmpty()) {
            Pair<Long, Long> current = queue.poll();
            Long controllerState = current.getFirst();
            Long plantState = current.getSecond();

            // Reintroducir self-loops incontrolables del estado correspondiente en plant
            for (Pair<String, Long> trans : plant.getTransitions(plantState, TransitionType.REQUIRED)) {
                String action = trans.getFirst();
                if(action.startsWith("c_"))
                    action = action.substring(2);
                Long dest = trans.getSecond();
                if (dest.equals(plantState) && cleanedLabels.contains(action)) {
                    // Reintroducir el self-loop en el controller
                    plainController.addAction(action);
                    plainController.addTransition(controllerState, action, controllerState, TransitionType.REQUIRED);
                }
            }

            // Avanzar a los estados hijos en ambas estructuras
            for (Pair<String, Long> controllerTrans : plainController.getTransitions(controllerState, TransitionType.REQUIRED)) {
                String action = controllerTrans.getFirst();
                Long nextControllerState = controllerTrans.getSecond();
                if (action.startsWith("c_")) {
                    // Remover la transición del plainController que es fake
                    plainController.removeRequired(controllerState, action, controllerTrans.getSecond());
                }
                // Buscar en plant una transición con la misma acción desde plantState
                for (Pair<String, Long> plantTrans : plant.getTransitions(plantState, TransitionType.REQUIRED)) {
                    if (plantTrans.getFirst().equals(action)) {
                        Long nextPlantState = plantTrans.getSecond();
                        Pair<Long, Long> nextPair = new Pair(nextControllerState, nextPlantState);
...
//            //System.out.println("Environment's " + ss.getState() + " is controller's " + mapping.get(ss));
    ...
    public static CompactState getStarEnv(CompactState c, LTSOutput output) {
        ...
        output.outln("Starred model for " + c.getName() + " generated in " + (System.currentTimeMillis() - initialTime)
                + "ms.");
        return MTSToAutomataConverter.getInstance().convert(plainController, c.getName(), true);
    }
        ...
        try {
            ...
            if (distributionResult.isDistributable()) {
                ...
                for (Entry<String, Set<String>> entry : componentsNameAndAlphabetEntrySet) {
                    ...
                    CompactState compactStateComponent = converter.convert(mTSComponentLongString, componentName, true);
        ...
        if (compositeState.getComposition() != null) {
            if (compositeState.getComponentAlphabet() != null) {
                compositeState.setComposition(getComponentModel(compositeState.getComposition(),
                        (Vector<String>) compositeState.getComponentAlphabet(), output));
            } else {
    ...
    private static CompactState getComponentModel(CompactState composition, Vector<String> componentAlphabet,
        ...
        return MTSToAutomataConverter.getInstance().convert(componentMTS, composition.getName(), true);
    }
    ...
    public static void makeProperty(CompositeState compositeState, LTSOutput output) {
        if (compositeState.getComposition() != null) {
            if (!MTSUtils.isMTSRepresentation(compositeState)) {
                ...
                CompactState compactState = (CompactState) converter.convert(mts, compositeState.name, true);
                compositeState.setComposition(compactState);
    ...
    public static void CompositionalApproach(CompositeState compositeState, final LTSOutput output, Statistics[] stats) {
        CompositionalApproach compositionalApproachSynthesizer = new CompositionalApproach();
        try {
            compositionalApproachSynthesizer.synthesize(compositeState, output);
        } catch (Exception e) {
     ...
     * @param stats
     */
    public static void synthesiseWithAnotherEngine(CompositeState compositeState,
                                                   boolean spectraEngine,
                                                   boolean strixEngine,
                                                   boolean monolithicTranslation,
                                                   final LTSOutput output,
                                                   MTSTools.ac.ic.doc.mtstools.model.operations.DCS.Compositional.Statistics stats) throws IOException {
        ...
        if (!(goal.getGuarantees().isEmpty() ^ goal.getMarking().isEmpty())) {
            output.outln("Marking or liveness requirements (exactly one of them) are required for heuristic analysis.");
            return;
        }
        if (!goal.getMarking().isEmpty() && goal.getAssumptions().size() > 1) {
            output.outln("Multiple assumptions are not supported by the heuristic analysis when using marking as goals.");
        ...
        // ltss
        List<MTS<Long, String>> ltss = new ArrayList<>();
        Set<String> actions = new HashSet<>();
        for (CompactState automata : compositeState.getMachines()) {
            MTS<Long, String> lts =
        ...
        if(monolithicTranslation){
            compositeState.compose(output);
            ltss = new ArrayList<>(
                    Collections.singleton(
                            AutomataToMTSConverter.getInstance().convert(compositeState.getComposition())));
        }
        ...
        if(spectraEngine){
            ...
            TranslateToSpectra<Long, String> translator = new TranslateToSpectra<Long, String>();
            translator.translate(ltss, goal.getControllableActions(), new ArrayList<>(goal.getAssumptions()), new ArrayList<>(goal.getGuarantees()));
            long ended = System.currentTimeMillis();
            System.out.println("\nTime translating Spectra: " + (ended-started) + "\n");
            stats.registerTimeToReduce(ended-started);
            // Creates a file named /tmp/filename.spectra
            String command = "java -jar /home/hernan/Escritorio/compositional/mtsa/spectra-cli.jar -i /tmp/filename.spectra";
            ...
            try {
                ...
                assertEquals("Spectra process Failed",0, exitCode);
...
//            TranslateToStrix<Long, String> translator = new TranslateToStrix<Long, String>();
//            String strixTranslationFormula = translator.binaryTranslate(ltss, goal.getControllableActions(), new HashSet<>(goal.getAssumptions()), new HashSet<>(goal.getGuarantees()));
//            String strixVariablesForCommand = translator.getVariablesOfCommandForStrix(ltss, goal.getControllableActions(), new HashSet<>(goal.getAssumptions()), new HashSet<>(goal.getGuarantees()));
//
//            // Create a temporary file in /tmp directory
//            File tmpFile = null;
//            try {
//                tmpFile = File.createTempFile("spec", ".strix", new File("/tmp"));
//            } catch (IOException e) {
//                throw new RuntimeException(e);
//            }
//
...
//                writer.write(strixTranslationFormula
//                                .replace("| ()",""));
//            } catch (IOException e) {
//                throw new RuntimeException(e);
//            }
//
//            String command= "strix -F " + tmpFile + " " + strixVariablesForCommand;
//            // Split the command into arguments
//            List<String> commandList = Arrays.asList(command.split("\\s+"));
//            ProcessBuilder processBuilder = new ProcessBuilder(commandList);
//
