...
//        executer.setPriority(Thread.NORM_PRIORITY - 1);
    ...
    private void exportFile() {
        ...
        fileChooser.setDialogTitle("Export as format");

        // Agregar filtros
        FileNameExtensionFilter xmlFilter = new FileNameExtensionFilter("XML (*.xml)", "xml");
        FileNameExtensionFilter autFilter = new FileNameExtensionFilter("Aldebaran (*.aut)", "aut");
        FileNameExtensionFilter smvFilter = new FileNameExtensionFilter("SMV (*.smv)", "smv");
        FileNameExtensionFilter slugsFilter = new FileNameExtensionFilter("Slugs (*.slugs)", "slugs");
        FileNameExtensionFilter pyFilter = new FileNameExtensionFilter("Python (*.py)", "py");
        FileNameExtensionFilter pddlFilter = new FileNameExtensionFilter("PDDL (*.pddl)", "pddl");
        FileNameExtensionFilter fspFilter = new FileNameExtensionFilter("FSP (*.fsp)", "fsp");

        fileChooser.addChoosableFileFilter(xmlFilter);
        fileChooser.addChoosableFileFilter(autFilter);
        fileChooser.addChoosableFileFilter(smvFilter);
        fileChooser.addChoosableFileFilter(slugsFilter);
        fileChooser.addChoosableFileFilter(pyFilter);
        fileChooser.addChoosableFileFilter(pddlFilter);
        fileChooser.addChoosableFileFilter(fspFilter);

        // Establecer el filtro por defecto
        FileNameExtensionFilter defaultFilter = current.isHeuristic ? xmlFilter : autFilter;
        fileChooser.setFileFilter(defaultFilter);

        // Establecer nombre sugerido
        String fname = current.composition == null ? current.name : current.composition.name;
        String defaultExt = current.isHeuristic ? ".xml" : ".aut";
        fileChooser.setSelectedFile(new File(fname + defaultExt));

        // Listener para cambiar el nombre sugerido cuando se cambia el filtro
        fileChooser.addPropertyChangeListener(JFileChooser.FILE_FILTER_CHANGED_PROPERTY, evt -> {
            FileNameExtensionFilter newFilter = (FileNameExtensionFilter) fileChooser.getFileFilter();
            ...
            fileChooser.setSelectedFile(new File(baseName + "." + newExt));
        });
        ...
        String selectedExt = ((FileNameExtensionFilter) fileChooser.getFileFilter()).getExtensions()[0];

        // Agregar la extensión si no está
        if (!file.getName().toLowerCase().endsWith("." + selectedExt)) {
        ...
        try (FileOutputStream fout = new FileOutputStream(file);
            ...
            String extension = "." + selectedExt.toLowerCase();
            ...
            ltsOutput.outln("Exported to: " + file.getAbsolutePath());
    ...
    class ExportFileAction implements ActionListener {
        public void actionPerformed(ActionEvent e) {
            String pp = textIO.getTitleAt(textIO.getSelectedIndex());
            if (pp.equals("Edit") || pp.equals("Transitions") || pp.equals("Output"))
                exportFile();
        }
    }
    ...
    private void doLegality() {
        if (compileIfChange() && current != null) {
            ...
            if (legalityOptions.isValid()) {
                ...
                Vector<String> controlledActions = new Vector<>(legalityOptions.actionSet);

                showOutput();
                ltsOutput.outln("==== Legality check ====");
                //List<String> actions = (List<String>) actionSet.actions;

                LegalityAnalyser a = new LegalityAnalyser(current, ltsOutput, legalityOptions.source, legalityOptions.target, controlledActions);
                a.composeNoHide();
...
//        lf = UIManager.getCrossPlatformLookAndFeelClassName(); // comment this before commiting ("fix" for dark mode in ubuntu)
        try {
