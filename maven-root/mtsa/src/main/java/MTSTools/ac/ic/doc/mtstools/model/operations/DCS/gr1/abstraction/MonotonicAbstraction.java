 ...
 */
public class MonotonicAbstraction<State, Action> implements OneCompostateAbstraction<State, Action> {
    ...
    private DirectedControllerSynthesisGR1<State, Action> dcs;
    /**
    ...
    public MonotonicAbstraction(DirectedControllerSynthesisGR1<State, Action> dcs) {
        this(dcs.ltss, dcs.defaultTargets, dcs.base, dcs.alphabet);
        this.dcs = dcs;
    }
    ...
    private void reconcilateCrossLTS(Compostate<State, Action> compostate) {
        for (int lts = 0; lts < ltss.size(); ++lts) {
            HState<State, Action> s = buildHState(lts, compostate.getStates().get(lts));
            for (HAction<Action> l : compostate.getAvailableActions()) {
    ...
    private Integer dist(HState<State, Action> source, HAction<Action> label) {
        if (!stateGenerations.containsKey(source) || !actionGenerations.containsKey(label))
            return DirectedControllerSynthesisGR1.INF;
    ...
    private Integer minDist(HState<State, Action> source, Set<HAction<Action>> labels) {
        int result = DirectedControllerSynthesisGR1.INF;
    ...
    private Integer addDist(Integer d1, Integer d2) {
        if (d1 == DirectedControllerSynthesisGR1.INF || d2 == DirectedControllerSynthesisGR1.INF)
            return DirectedControllerSynthesisGR1.INF;
        return d1 + d2;
    }
        ...
        for (int lts = 0; lts < ltss.size(); ++lts) {
            for (State state : ltss.get(lts).getStates()) {
                ...
                for (Pair<Action, State> transition : ltss.get(lts).getTransitions(state)) {
                    ...
                    if (newDist != DirectedControllerSynthesisGR1.INF) { //!!!
                        DirectedControllerSynthesisGR1.putmin(manyStepsToDestination, label, newDist);
