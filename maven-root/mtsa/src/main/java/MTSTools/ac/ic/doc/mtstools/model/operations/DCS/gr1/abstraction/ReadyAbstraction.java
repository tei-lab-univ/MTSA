...
import MTSTools.ac.ic.doc.mtstools.model.operations.DCS.gr1.*;

import java.util.*;

/** This class implements the Ready Abstraction (RA). */
public class ReadyAbstraction<State, Action> implements OneCompostateAbstraction<State, Action> {
    ...
    private void evaluateRA(Compostate<State, Action> compostate) {
        for (int lts = 0; lts < ltss.size(); ++lts) {
            ...
            for (Pair<Action,State> transitions : s.getTransitions()) {
                ...
                if (t.equals(-1L))
                    continue;
                int mt = 2, dt = DirectedControllerSynthesisGR1.INF;
                ...
                if (newlDist.compareTo(currentDist) < 0) {
                    ...
                    fresh.add(l);
                    if (compostate.getAvailableActions().contains(l)) // register only the shortest distances for enabled events
        ...
        while (!fresh.isEmpty()) {
            ...
            for (HAction<Action> l : edges.getK(t)) {
                ...
                for (int lts = 0; lts < ltss.size(); ++lts) {
                    ...
                    HDist newlDist = dl == DirectedControllerSynthesisGR1.INF ? HDist.chasm : getHDist(tDist.getFirst(), dl);
                    if (newlDist.compareTo(lDist) < 0) {
                        estimates.get(l).set(lts, newlDist);
                        fresh.add(l);
                        if (compostate.getAvailableActions().contains(l))
                            registerShort(lts, newlDist);
                    }
                }
            }
    ...
    private Integer gap(Compostate<State, Action> compostate, HAction<Action> l, HAction<Action> t) {
        ...
        for (Integer lts : actionsToLTS.get(l)) {
            ...
            if (s.contains(l)) {
                ...
                dl = dl == null ? DirectedControllerSynthesisGR1.INF : dl - 1;
                if (dl > result)
                    result = dl;
            } else {
                score = 1;
        ...
        if (result != DirectedControllerSynthesisGR1.INF)
            result += score;
    ...
    private Integer addDist(Integer d1, Integer d2) {
        return (d1 == DirectedControllerSynthesisGR1.INF || d2 == DirectedControllerSynthesisGR1.INF) ? DirectedControllerSynthesisGR1.INF : d1 + d2;
    }
    ...
    private void reconcilateShort(Compostate<State, Action> compostate) {
        ...
        for (HAction<Action> l : compostate.getAvailableActions()) { // this loops fills missing goals with shortest paths
    ...
    private void extractRecommendations(Compostate<State, Action> compostate) {
        ...
        HashSet<HAction<Action>> explored = new HashSet<>();
        for (Pair<HAction<Action>, Compostate<State, Action>> transition : compostate.getExploredChildren()) {
