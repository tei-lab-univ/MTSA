...
import MTSTools.ac.ic.doc.mtstools.model.operations.DCS.Compositional.FormulaUtils;
...
public class ControllerGoal<Action> implements Cloneable {
	...
	private Set<String> selfLoopsUncontrollable = new HashSet<>();
	...
	public ControllerGoal() {
		...
		this.selfLoopsUncontrollable = new HashSet<>();
	}
	...
	public Object clone() throws CloneNotSupportedException {
		...
		List<Formula> assumptionsCopy = new ArrayList<>();
		List<Formula> guaranteesCopy = new ArrayList<>();
		Set<Fluent> fluentsCopy = new HashSet<>();

		for(Formula assumption : this.assumptions){
			Formula newFormula = FormulaUtils.copyFormula(assumption, fluentsCopy);
			assumptionsCopy.add(newFormula);
		}
		for(Formula guarantee : this.guarantees){
			Formula newFormula = FormulaUtils.copyFormula(guarantee, fluentsCopy);
			guaranteesCopy.add(newFormula);
		}
		clone.assumptions = assumptionsCopy;
		clone.guarantees = guaranteesCopy;
		clone.fluents = fluentsCopy;

		clone.buchi = this.buchi;

		clone.controllableActions = new HashSet<>(this.controllableActions);
		...
		clone.selfLoopsUncontrollable = this.selfLoopsUncontrollable;
		return clone;
	}
	...
	public void setAssumptions(List<Formula> assumptions) { this.assumptions = assumptions; }

	public List<Formula> getGuarantees() {
		return guarantees;
	}
	public void setGuarantees(List<Formula> guarantees) { this.guarantees = guarantees; }
	...
	public Set<String> getSelfLoopsUncontrollable(){ return selfLoopsUncontrollable; }
	public void setSelfLoopsUncontrollable(Set<String> setLoops){ this.selfLoopsUncontrollable = setLoops; }
