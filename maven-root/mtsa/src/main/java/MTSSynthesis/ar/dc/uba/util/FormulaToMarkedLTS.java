...
public class FormulaToMarkedLTS {
	...
	public MarkedLTSImpl<Long, String> translate(Formula formula) {
		...
		if (formula instanceof FluentPropositionalVariable) {
			result = fluentToMarkedLTSImpl(((FluentPropositionalVariable) formula).getFluent());
	...
	private MarkedLTSImpl<Long, String> fluentToMarkedLTSImpl(Fluent fluent) {
		boolean initial = fluent.getInitialValue();
		MarkedLTSImpl<Long, String> result = new MarkedLTSImpl<Long, String>(initial ? 1L : 0L, fluent);
		result.addState(0L);
		result.addState(1L);
